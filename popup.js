...
function startScan(min, r1min, r1max, r2min, r2max, detectionMethod) {
    ...
    function detectComments(post) {
        // STRICT OVERRIDE: Check if the post explicitly states there are no comments
        const fullTextRaw = (post.innerText || "").toLowerCase();
        if (fullTextRaw.includes('no comments yet') || 
            fullTextRaw.includes('be the first to comment') || 
            fullTextRaw.includes('কোনো মন্তব্য নেই') || 
            fullTextRaw.includes('এখনো কোনো মন্তব্য নেই') ||
            fullTextRaw.includes('कोई टिप्पणी नहीं')) {
            return 0;
        }

        if (detectionMethod === 'robust') {
            // METHOD 2: DEEP TEXT SCAN (TreeWalker) - Check nodes individually to avoid mashing Likes + Comment label
            ...
            for (const el of elements) {
                ...
                const containsCommentKeyword = label.includes('comment') || label.includes('মন্তব্য') || 
                                               ...
                                               label.includes('टिप्पणियाँ') || text.includes('टिप्पणियाँ');
                ...
                if (containsCommentKeyword && !isLikeOrShare) {
                    if (label.includes('write') || label.includes('লিখুন') || text.includes('write') || text.includes('লিখুন') || 
                        label.includes('first') || text.includes('first') || 
                        label.includes('no comment') || text.includes('no comment') || 
                        label.includes('yet') || text.includes('yet')) continue;

                    let regex = /([\d,]+(?:\.\d+)?[kKmM]?)\s*(?:comments?|মন্তব্য|टिप्पणियाँ)/i;
                    
                    let countMatch = label.match(regex);
                    if (countMatch) {
                        let count = parseNumber(countMatch[1]);
                        if (count > 0) return count;
                    }

                    countMatch = text.match(regex);
                    if (countMatch) {
                        let count = parseNumber(countMatch[1]);
                        if (count > 0) return count;
                    }

                    // Fallback for isolated numbers inside a valid comment container
                    let parsedOnlyNum = text.match(/^([\d,]+(?:\.\d+)?[kKmM]?)$/);
                    if (parsedOnlyNum) {
                        let count = parseNumber(parsedOnlyNum[1]);
                        if (count > 0) return count;
                    }
                }
            }

            // Safe Fallback: Check individual text nodes in the post for explicit comment counts
            // This prevents mashing independent text nodes (like '24' and 'Comment') into '24 Comment'
            let walker = document.createTreeWalker(post, NodeFilter.SHOW_TEXT, null, false);
            let textNode;
            let fallbackRegex = /^([\d,]+(?:\.\d+)?[kKmM]?)\s*(?:comments?|মন্তব্য|टिप्पणियाँ)$/i;
            while ((textNode = walker.nextNode())) {
                let val = textNode.nodeValue.trim();
                if (val) {
                    let patternMatch = val.match(fallbackRegex);
                    if (patternMatch) return parseNumber(patternMatch[1]);
                    
                    // Specific match for "view X comments"
                    let viewMatch = val.match(/^view\s+([\d,]+(?:\.\d+)?[kKmM]?)\s*comments?$/i);
                    if (viewMatch) return parseNumber(viewMatch[1]);
                }
            }
    ...
    function getPostURL(post) {
        ...
        // Step 1: Detect video links and Identify Author Info
        for (let a of allAnchors) {
            let href = a.href.toLowerCase();
            
            // STRICT VIDEO REJECTION - if the post contains any video links, reject it immediately
            if (href.includes('/video') || href.includes('/watch') || href.includes('/reel') || href.includes('/live')) {
                return null;
            }

            // Skip common non-profile links for author detection
            if (href.includes('/posts/') || href.includes('/permalink') || href.includes('/photos') || href.includes('/photo') || href.includes('/groups/') || href.includes('story_fbid')) continue;
            
            if (href.includes('profile.php?id=')) {
                authorId = new URL(a.href).searchParams.get('id');
                // Don't break, so we keep checking for video links
            } else if (!authorUsername) {
                let match = href.match(/facebook\.com\/([^\/\?]+)/);
                if (match && !['photo.php', 'photo', 'permalink', 'story.php', 'groups', 'watch', 'share', 'events', 'hashtag', 'profile', 'pages', 'stories', 'reel', 'videos', 'ads', 'gaming', 'marketplace', 'home', 'notifications', 'friends', 'groups', 'messages', 'bookmarks'].includes(match[1])) {
        ...
        let finalCheck = urlResult.toLowerCase();
        if (finalCheck.includes('/video') || finalCheck.includes('/watch') || finalCheck.includes('/reel') || finalCheck.includes('/live')) {
            return null;
        }
    ...
    async function infiniteScroll() {
        ...
        while (running) {
            ...
            // CONSTANT TIME OPTIMIZATION: Only process the last 80 posts to prevent CPU freeze as DOM grows
            let allPosts = Array.from(document.querySelectorAll('div[role="article"], div[role="feed"] > div, div.x1yztbdb, div[data-pagelet]'));
            let posts = allPosts.slice(-80);
